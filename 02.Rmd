...
head(d)
svg_file <- 'Z:/GIT/stat_rethinking/chapters/inst/salvelinus-confluentus-bull-trout.svg'
svg_file2 <- 'Z:/GIT/stat_rethinking/chapters/inst/oncorhynchus-mykiss-rainbow-trout.svg'
...
Within the `rethinking` package, @mcelreath2018statistical has a function `quap` that does this for us. We just need to provide the necessary information and the function does the rest.  
...
From the output we see summary stats from the approximation. We can already see with our data that the bounds extend beyond what is theoretically possible. Most of the time this is a problem and is where you really need to think about the assumptions of your data and what you want exactly out of your estimand. If we compare it to a `beta` distribution, you can see that it fits much better 

```{r, warning=FALSE, message=FALSE}
beta_qa <- quap(
alist(
S ~ dbetabinom(size = 9, prob = p, theta = exp(t)) , # beta binomial likelihood
p ~ dunif(0,1),
t ~ dnorm(1,10)# uniform prior
) ,
data=list(S=1,L=8) )
# display summary of quadratic approximation
...
In the figure above, we can see that it does a decent job finding the shape but since it assumes Gaussian it's tails are thin as well as beyond the bounds of the parameter!  

Now make Figure 2.8.

```{r, eval = FALSE, fig.width = 8, fig.height = 2.75}
n_grid <- 100
